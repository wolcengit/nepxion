...
public class TGraphManager
{
	...
	public static void delete(TGraph graph)
	{
		...
		{
			int selectedValue = JBasicOptionPane.showConfirmDialog(HandleManager.getFrame(graph), "确认删除所选的图元？", SwingLocale.getString("confirm"), JBasicOptionPane.YES_NO_OPTION);
			if (selectedValue == JBasicOptionPane.OK_OPTION)
			{
				...
				for (Iterator iterator = allSelectedElement.iterator(); iterator.hasNext();)
				{
					...
					{
						JBasicOptionPane.showMessageDialog(HandleManager.getFrame(graph), "节点“" + element.toString() + "”不可被删除", "错误", JBasicOptionPane.WARNING_MESSAGE);
	...
	public static void clear(TGraph graph)
	{
		...
		{
			int selectedValue = JBasicOptionPane.showConfirmDialog(HandleManager.getFrame(graph), "确认清空所有的图元？", SwingLocale.getString("confirm"), JBasicOptionPane.YES_NO_OPTION);
			if (selectedValue == JBasicOptionPane.OK_OPTION)
			{
	...
	public static void toggleControlBar(TGraph graph, boolean isVisible)
	{
		...
		if (controlBarInternalFrame == null)
		{
			...
			controlBar.setPreferredSize(new Dimension(90, 300));
			
			controlBarInternalFrame = new TInternalFrame(graph, "面板", TIconFactory.getContextIcon("pane.png"), controlBar);
	...
	public static void scale(TGraph graph)
	{
		...
		if (scaleInternalFrame == null)
		{
			...
			scaleInternalFrame = new TInternalFrame(graph, "坐标和比例", TIconFactory.getContextIcon("scale.png"), scaleBar);
	...
	public static void overview(TGraph graph)
	{
		...
		if (overview == null)
		{
			overview = new TOverview(graph, "鹰眼", new Dimension(150, 150), TIconFactory.getContextIcon("overview.png"));
	...
	public static void layout(TGraph graph)
	{
		...
		if (layoutInternalFrame == null)
		{
			...
			layoutInternalFrame = new TInternalFrame(graph, "布局器", TIconFactory.getContextIcon("layout.png"), layouterBar);
	...
	public static void importXML(final TGraph graph)
	{
		...
		if (fileSelector == null)
		{
			...
			fileSelector = new JFileSelector(HandleManager.getFrame(graph), "导入数据文件XML");
		...
		if (file != null)
		{
			...
			{
				int selectedValue = JBasicOptionPane.showConfirmDialog(HandleManager.getFrame(graph), "视图上已有图元存在。是否需要清除已存在的图元？", SwingLocale.getString("confirm"), JBasicOptionPane.YES_NO_CANCEL_OPTION);
				if (selectedValue == JBasicOptionPane.OK_OPTION)
				{
					graph.getDataBox().clear();
				}
			...
			JThreadDialog dialog = new JThreadDialog(HandleManager.getFrame(graph), "导入数据文件XML", SwingLocale.getString("export_and_wait"))
			{
					...
					throws Exception
				{
					...
					JBasicOptionPane.showMessageDialog(HandleManager.getFrame(graph), "导入数据文件XML", SwingLocale.getString("information"), JBasicOptionPane.INFORMATION_MESSAGE);
				}
	...
	public static void exportXML(final TGraph graph)
	{
		...
		if (fileSelector == null)
		{
			...
			fileSelector = new JFileSelector(HandleManager.getFrame(graph), "导出数据文件XML");
			fileSelector.addChoosableFileFilter(xmlFileFilter);
			fileSelector.setFileFilter(xmlFileFilter);
		...
		if (filePath != null)
		{
			JThreadDialog dialog = new JThreadDialog(HandleManager.getFrame(graph), "导出数据文件XML", SwingLocale.getString("export_and_wait"))
			{
				protected void loadForeground(Object data)
					throws Exception
				{
					JBasicOptionPane.showMessageDialog(HandleManager.getFrame(graph), "导出数据文件XML成功", SwingLocale.getString("information"), JBasicOptionPane.INFORMATION_MESSAGE);
				}
	...
	public static void exportSVG(final TGraph graph)
	{
		...
		{			
			...
			fileSelector = new JFileSelector(HandleManager.getFrame(graph), "导出矢量文件SVG");
		...
		if (filePath != null)
		{
			JThreadDialog dialog = new JThreadDialog(HandleManager.getFrame(graph), "导出矢量文件SVG", SwingLocale.getString("export_and_wait"))
			{
				protected void loadForeground(Object data)
					throws Exception
				{
					JBasicOptionPane.showMessageDialog(HandleManager.getFrame(graph), "导出矢量文件SVG成功", SwingLocale.getString("information"), JBasicOptionPane.INFORMATION_MESSAGE);
				}
	...
	public static void exportImage(final TGraph graph)
	{
		...
		int selectedValue = JBasicOptionPane.showDialog(HandleManager.getFrame(graph), exportImageConfig, "图片参数设置", IconFactory.getSwingIcon("banner/image.png"));
		...
		if (fileSelector == null)
		{
			...
			fileSelector = new JFileSelector(HandleManager.getFrame(graph), "导出图片文件");
		...
		if (filePath != null)
		{
			JThreadDialog dialog = new JThreadDialog(HandleManager.getFrame(graph), "导出图片文件", SwingLocale.getString("export_and_wait"))
			{
				protected void loadForeground(Object data)
					throws Exception
				{
					JBasicOptionPane.showMessageDialog(HandleManager.getFrame(graph), "导出图片文件成功", SwingLocale.getString("information"), JBasicOptionPane.INFORMATION_MESSAGE);
				}
